...
const POS = {
  ...
  퍼스트포스: {
    ...
    idCol: '상품코드', emitHeader: true, confirmed: true,   // 팀 확정 간주(실제 성공파일 확보 시 defaults 재점검 권장)
  ...
  스파로스포스: {
    ...
    idCol: 'ERP상품코드', emitHeader: false, confirmed: true,   // 팀 확정 간주 · ⚠️ 데이터는 공식양식 4행부터 → 데이터행만 생성
...
const uploadRows = [], tsvRows = [], checklist = [], warns = [], reviewItems = [];
...
for (const c of changes) {
  ...
  if (c.op !== 'delete' && !priceMissing) {      // 삭제·가격미판독은 양식 제외 → 체크리스트에만
    ...
    tsvRows.push(cfg.cols.map(h => String(rowObj[h] ?? '').replace(/[\t\n]/g, ' ')).join('\t'));   // 붙여넣기용
  }
...
const tsvPath = path.join(outDir, `${posKey}_붙여넣기_${safe}.tsv`);
...
console.log(`[③ 붙여넣기 TSV]    ${tsvPath}`);
if (cfg.pasteNote) console.log('※ ' + cfg.pasteNote);
console.log('\n─── 📋 엑셀 양식 붙여넣기용 텍스트 (데이터 첫 행 첫 칸에 Ctrl+V) ───');
